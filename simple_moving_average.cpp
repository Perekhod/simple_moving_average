...
std::vector<T> simple_moving_average(const std::vector<T>& data, int window_size) {
    // Проверяем, что размер окна не больше количества элементов в векторе данных
    if (window_size > data.size()) {
        std::cout << "Ошибка ручного тестирования:\n"
            << "Размер окна не может быть больше количества элементов в векторе.\n"
            << "Продолжение работы программы...\n";
        return std::vector<T>(); // Возвращаем пустой вектор
    }
...
int main() {
    ...
    // Вычисляем SMA
    std::vector<double> sma = simple_moving_average(data, 50);
    // Выводим результаты
    for (double val : sma) {
        std::cout << val << " ";
    }
